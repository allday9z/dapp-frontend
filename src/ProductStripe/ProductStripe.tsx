...
export const ProductStripe: FC<ProductStripeProps> = ({
  ...
  const scrollPage = useCallback((dir: 1 | -1) => {
    const el = viewRef.current;
    if (!el) return;
    // Page = visible width minus one tile overlap (~120px)
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    ...
    viewRef.current.scrollLeft = scrollStart.current - (e.touches[0].pageX - startX.current);
  };

  return (
    <div className={`product-stripe ${className}`}>
      {/* scrollable viewport */}
      <div
        ref={viewRef}
      ...
      >
        <div className="product-stripe__inner">
          {children}
        </div>
      </div>
